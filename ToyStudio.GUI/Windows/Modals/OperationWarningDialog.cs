...
namespace ToyStudio.GUI.Windows.Modals
{
    class OperationWarningDialog : IPopupModal<OperationWarningDialog.DialogResult>
    {
        ...
        {
            _warning = warning;
            _categorizedWarnings = categorizedWarnings;
        }
        ...
        public void DrawModalContent(Promise<DialogResult> promise)
        {
            ...
            if (ImGui.BeginChild("Categorized", Vector2.Zero, ImGuiChildFlags.AutoResizeY))
            {
                ...
                foreach (var (category, warnings) in _categorizedWarnings)
                {
                    if (warnings.Count == 0)
                        continue;
                    float offset = 0;
        ...
        private readonly (string category, IReadOnlyList<string> warnings)[] _categorizedWarnings;
    }
}

    ...
    public class PopupModalHost : IPopupModalHost
    {
        ...
        public void DrawHostedModals()
        {
            ...
            foreach (var (info, methods, resultTask) in mPopupStack.Reverse())
            {
                ...
                Vector2 center = ImGuiNET.ImGui.GetMainViewport().GetCenter();
                ImGuiNET.ImGui.SetNextWindowPos(center, ImGuiCond.Always, new Vector2(0.5f, 0.5f));
                if (info.MinWindowSize.TryGetValue(out Vector2 minWindowSize))
                    ImGuiNET.ImGui.SetNextWindowSizeConstraints(minWindowSize, ImGuiNET.ImGui.GetWindowViewport().Size);

                if (!shouldClose && !ImGuiNET.ImGui.IsPopupOpen(info.Id))
                    ImGuiNET.ImGui.OpenPopup(info.Id);

                bool open = true;
                if (!ImGuiNET.ImGui.BeginPopupModal(info.Id, ref open, info.WindowFlags))
                {
                    methods.Cancel();
                    continue;
                }
